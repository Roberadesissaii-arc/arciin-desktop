...
`src-tauri/tests/secret_safety.rs` needs no server: it asserts the
trusted-device credential reaches exactly one `Authorization` header, never a
URL, never a logging macro, and never the state handed to the UI.

Pairing and revocation still need a live server, and a disposable one: a
device that is paired and then revoked from Settings → Devices, with the client
running, is what proves the revocation watchdog.
